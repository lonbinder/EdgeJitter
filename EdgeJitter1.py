...
class JitterProcessor:
    ...
    def getUserInputSize(self):
        cmdDef = _ui.commandDefinitions.itemById(self.cmdId)
        if not cmdDef:
    ...
    def recursiveCut(self, selectedLine, startPoint, endPoint, dominantAxis, minSize, maxSize):
        ...
        newSelectedCurves = createFunction(sketch, selectedLine, startPoint, endPoint, dominantAxis, maxSize, direction)
        ...
        for recurseCurve in newSelectedCurves:
            ...
            if recurseCurve.length < (maxSize * 3):
                ...
                self.recursiveCut(recurseCurve, recurseCurveStartPoint, 
                        recurseCurveEndPoint, dominantAxis, minSize, maxSize)
        ...
        def notify(self, args):
            if args.action == 'DOMContentLoaded':
                ...
                try:
                    ...
                    minSize = float(data.get('minSize'))
                    maxSize = float(data.get('maxSize'))
                    if minSize <= 0 or maxSize >= 100 or minSize > maxSize:
                        ...
                        self.parent.callbackFromSizeInput(minSize, maxSize)
